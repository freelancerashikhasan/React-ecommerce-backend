...
trait StockTrait {
    ...
    public function sell_qty($product_id, $agent){
        $totalQty = Invoice::leftJoin('invoice_items', 'invoices.id', '=', 'invoice_items.invoice_id')->where('invoices.agent_id', $agent)->whereIn('invoices.type', [Constant::ORDER_TYPE['customer'], Constant::ORDER_TYPE['customer_repurchase']])->whereIn('invoices.status', [Constant::STATUS['pending'], Constant::STATUS['approved']])->where('invoices.order_status', '!=' ,Constant::ORDER_STATUS['rejected'])->where('invoice_items.product_id', $product_id)->sum('invoice_items.qty');
        return $totalQty ?? 0;
    }

    public function package_sell_qty($package_id, $agent){
        $totalQty = Invoice::leftJoin('invoice_items', 'invoices.id', '=', 'invoice_items.invoice_id')->where('invoices.agent_id', $agent)->whereIn('invoices.type', [Constant::ORDER_TYPE['customer_packege'], Constant::ORDER_TYPE['customer_repurchase']])->whereIn('invoices.status', [Constant::STATUS['pending'], Constant::STATUS['approved']])->where('invoices.order_status', '!=' ,Constant::ORDER_STATUS['rejected'])->where('invoice_items.package_id', $package_id)->sum('invoice_items.qty');
        return $totalQty ?? 0;
        // pending
    ...
    public static function agent_product_stock($product_id, $agent, $user_id){
        ...
        $sell_qty = Invoice::leftJoin('invoice_items', 'invoices.id', '=', 'invoice_items.invoice_id')->where('invoices.agent_id', $agent)->whereIn('invoices.type', [Constant::ORDER_TYPE['customer'], Constant::ORDER_TYPE['customer_repurchase']])->whereIn('invoices.status', [Constant::STATUS['pending'], Constant::STATUS['approved']])->where('invoices.order_status', '!=' ,Constant::ORDER_STATUS['rejected'])->where('invoice_items.product_id', $product_id)->sum('invoice_items.qty') ?? 0;
    ...
    public function companyProductTotalQTY($product_id){
        $totalQty = ProductStock::where('product_id', $product_id)->sum('qty');
        return $totalQty ?? 0;
    }
    public function companyProductSale($product_id){
        $totalQty = InvoiceItem::where('product_id', $product_id)->sum('qty');
        return $totalQty ?? 0;
    }
    public function companyProductStock($product_id){
        return ($this->companyProductTotalQTY($product_id) - $this->companyProductSale($product_id)) ?? 0;
    }

    public static function productStock($product_id){
        $totalQty = ProductStock::where('product_id', $product_id)->sum('qty') ?? 0;
        $totalSales = InvoiceItem::where('product_id', $product_id)->sum('qty') ?? 0;

        return ($totalQty - $totalSales) ?? 0;
    }
}
