    ...
    <script>
        ...
        function addCountry(){
            ...
            $.ajax({
                ...
                url: "{{ route('admin.settings.upzila.store') }}",
                success: function(res) {
                    resetForm();
                    success_msg('Upzila Added Successfully');
                    $('.country_close').click();
                ...
                error: function(error) {
                    ...
                    if (errors.district_id) {
                        $('.district_Error').text(errors.district_id);
                        $('#district').addClass('border-danger is-invalid');
                        $('#district').val('').trigger('change');
        ...
        function edit(id){
            var url = "{{ route('admin.settings.upzila.edit', ':id') }}";
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('#row_id').val(data.id);
                    $('#division').val(data.district.division_id).trigger('change');
        ...
        function UpdateCountry(){
            ...
            var route = "{{ route('admin.settings.upzila.update', ':id') }}";
            ...
            $.ajax({
                ...
                success: function(res) {
                    resetForm();
                    success_msg('Upzila Updated Successfully');
                    $('.country_close').click();
                    $('#datatable').DataTable().ajax.reload();
                },
                error: function(error) {
                    ...
                    if (errors.district_id) {
                        $('.district_Error').text(errors.district_id);
                        $('#district').addClass('border-danger is-invalid');
                        $('#district').val(errors.district_id).trigger('change');
                        error('' + errors.district_id )
                    }
                    if (errors.name) {
                        $('.name_en_Error').text(errors.name);
        ...
        // Category Delete
        function destroy(id){
            var url = "{{ route('admin.settings.upzila.destroy', ':id') }}";
            url = url.replace(':id', id);
            Swal.fire({
                title: "Are you sure?",
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if(data === 'not_delete'){
                                    ...
                                    success_msg('Upzila Deleted Successfully.');
                                    $('#datatable').DataTable().ajax.reload();
                                }
                            },
                            error: function(){
                                warning_msg('Upzila Not Found!');
                            }
                        });
                    }
            });
