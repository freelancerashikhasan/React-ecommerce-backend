...
trait Balance
{
    ...
    public static function available_balance(){
        ...
        $total = Transaction::where('user_id', $user_id)->where('status', Constant::STATUS['approved'])->where('wallet_type', Constant::WALLET_TYPE['active_balance'])->whereIn('transaction_type', [Constant::TRANSACTION_TYPE['customer_order_commission'], Constant::TRANSACTION_TYPE['pharmacy_order_commission'], Constant::TRANSACTION_TYPE['withdraw']])->select(DB::raw('ifnull(sum(cred_amount - deb_amount),0) as total'))->get();
        return $total[0]->total;
    }
    public static function self_commission(){
        $user_id = Auth::user()->id;
        $total = Transaction::where('user_id', $user_id)->where('status', Constant::STATUS['approved'])->where('wallet_type', Constant::WALLET_TYPE['active_balance'])->where('transaction_type', Constant::TRANSACTION_TYPE['self_entry_commission'])->select(DB::raw('ifnull(sum(cred_amount - deb_amount),0) as total'))->get();
        return $total[0]->total;
    }
    public static function team_commission(){
        $user_id = Auth::user()->id;
        $total = Transaction::where('user_id', $user_id)->where('status', Constant::STATUS['approved'])->where('wallet_type', Constant::WALLET_TYPE['active_balance'])->where('transaction_type', Constant::TRANSACTION_TYPE['group_entry_commission'])->select(DB::raw('ifnull(sum(cred_amount - deb_amount),0) as total'))->get();
        return $total[0]->total;
    }
    ...
    public static function customerPurchaseCommission(){
        $user_id = Auth::user()->id;
        $total = Transaction::where('user_id', $user_id)->where('status', Constant::STATUS['approved'])->where('wallet_type', Constant::WALLET_TYPE['active_balance'])->where('transaction_type', Constant::TRANSACTION_TYPE['customer_order_commission'])->select(DB::raw('ifnull(sum(cred_amount - deb_amount),0) as total'))->get();
        return $total[0]->total;
    }

    public static function pharmacyPurchaseCommission(){
        $user_id = Auth::user()->id;
        $total = Transaction::where('user_id', $user_id)->where('status', Constant::STATUS['approved'])->where('wallet_type', Constant::WALLET_TYPE['active_balance'])->where('transaction_type', Constant::TRANSACTION_TYPE['pharmacy_order_commission'])->select(DB::raw('ifnull(sum(cred_amount - deb_amount),0) as total'))->get();
        return $total[0]->total;
    }
